...
<script type="text/javascript">
	...
	function doSub(){
		//必填项验证
		$("#myModalLabel").html("信息验证");
		$("#modalFooter2").html("<button type='button' class='btn btn-primary' data-dismiss='modal' name='yes'>知道了</button>");
		$("button[name='yes']").click(function(){
			$("#myModal").modal('hide');
		});
		if($("input[name='hyName']").val()==""){
			$("#modalBody").html("会员名称不能为空");
			$("#myModal").modal('show');
			return;
		}
		if($("input[name='hyNo']").val()==""){
			$("#modalBody").html("会员代号不能为空");
			$("#myModal").modal('show');
			return;
		}
		if($("input[name='hyPhone']").val()==""){
			$("#modalBody").html("会员电话不能为空");
			$("#myModal").modal('show');
			return;
		}
	...
	function tijiao(){
		$("form[name='infoForm']").attr("action","<%=ctx%>/huiyuan/save.action");
		$("form").submit();
	}
