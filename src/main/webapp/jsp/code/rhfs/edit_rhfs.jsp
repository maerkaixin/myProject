...
<html>
	...
	<body style="background:linear-gradient(white,#ebebeb,white);">
		<label class="control-label label-default col-xs-12 " style="margin-bottom: 30px;">
			<h4>当前位置：入会方式 － 入会方式</h4>
		</label>
		<div class="container">
			<hr>
			<form class="form-horizontal text-center" name="infoForm" action="javascript:doSub()" method="post">
				<input type="hidden" name="crfId" value="${rhfs.crfId}">
				<div class="container ">
					<div class="form-group">
						<label class="control-label col-sm-2 col-md-2">入会方式名称:</label>
						<div class="col-md-9 col-sm-9">
							<input class="form-control" name="crfName" value="${rhfs.crfName}" placeholder="请输入入会方式名称" />
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-sm-2 col-md-2">入会方式编码:</label>
						<div class="col-md-9 col-sm-9">
							<input class="form-control" name="crfCode" value="${rhfs.crfCode}" placeholder="请输入入会方式编码" />
						</div>
					</div>
					<div class="form-group">
...
<script type="text/javascript">
	...
	function tijiao(){
		$("form[name='infoForm']").attr("action","<%=ctx%>/rhfs/edit.action");
		$("form").submit();
	}
