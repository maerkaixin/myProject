...
<html>
	...
	<body style="background:linear-gradient(white,#ebebeb,white);">
		<label class="control-label label-default col-xs-12 " style="margin-bottom: 30px;">
			<h4>当前位置：爱好类信－ 查询爱好类型</h4>
		</label>
		<div class="container">
			<form method="post" action="<%=ctx %>/aihao/query.action" name="searchForm">
				<%-- 此处放条件查询 --%>
			</form>
		</div>
		<form action="<%=ctx %>/aihao/delete.action" method="post" name="delForm">
		<div class="container ">
			<table class="table table-hover table-striped table-condensed text-center" >
				...
				<tbody>
				<c:forEach items="${list }" var="aihao" varStatus="i">
					<tr>
						<td>${i.count}</td>
						<td><input type="checkbox" name="ids" value="${aihao.cahId}"></td>
						<td>${aihao.cahName}</td>
						<td>${aihao.cahCode}</td>
						<td>
							<button onclick="editpage('${aihao.cahId}')" type="button" class="btn-sm btn-primary">修改</button>
						</td>
					</tr>
					</c:forEach>
				</tbody>
...
<script type="text/javascript">
	function addpage(){
		window.location.href = "<%=ctx%>/aihao/addpage.action";
	}
	
	function editpage(id){
		window.location.href = "<%=ctx%>/aihao/editpage.action?id="+id;
	}
