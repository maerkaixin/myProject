...
<html>
	...
	<body style="background:linear-gradient(white,#ebebeb,white);">
		<label class="control-label label-default col-xs-12 " style="margin-bottom: 30px;">
			<h4>当前位置：工程类型 － 查询工程类型</h4>
		</label>
		<div class="container">
			<form method="post" action="<%=ctx %>/gclx/query.action" name="searchForm">
				<%-- 此处放条件查询 --%>
			</form>
		</div>
		<form action="<%=ctx %>/gclx/delete.action" method="post" name="delForm">
		<div class="container ">
			<table class="table table-hover table-striped table-condensed text-center" >
				...
				<tbody>
				<c:forEach items="${list }" var="gclx" varStatus="i">
					<tr>
						<td>${i.count}</td>
						<td><input type="checkbox" name="ids" value="${gclx.cglId}"></td>
						<td>${gclx.cglName}</td>
						<td>
							<button onclick="editpage('${gclx.cglId}')" type="button" class="btn-sm btn-primary">修改</button>
						</td>
					</tr>
					</c:forEach>
				</tbody>
...
<script type="text/javascript">
	function addpage(){
		window.location.href = "<%=ctx%>/gclx/addpage.action";
	}
	
	function editpage(id){
		window.location.href = "<%=ctx%>/gclx/editpage.action?id="+id;
	}
