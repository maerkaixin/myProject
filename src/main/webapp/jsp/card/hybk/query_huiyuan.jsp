...
<html>
	...
	<body style="background:linear-gradient(white、#ebebeb,white);">
		...
		<div class="container">
			<hr />
			<form name="queryForm" class="form-horizontal text-center" action="<%=ctx %>/huiyuan/getHuiyuan.action?pageNo=1" method="post">
				<div class="container">
			...
			<table class="table table-hover table-striped  text-center" >
				...
				<c:forEach items="${pageInfo.list }" var="hy" varStatus="i">
					<tr>
						...
						<td>${hy.hyXuexing}</td>
						<td>
							<a href="javascript:xuanze('${hy.hyId }','${hy.hyName }','${hy.hyNo }')">选择</a>&nbsp;&nbsp;
						</td>
					</tr>
					</c:forEach>
				</tbody>
		...
		<div align="center">
			<page:page pageName="pageInfo" uri="/huiyuan/getHuiyuan.action"></page:page>
		</div>
	</body>
</html>
<script type="text/javascript">
	function xuanze(hyId,hyName,hyNo){
		$(parent.document).find("input[name='hyId']").val(hyId);
		$(parent.document).find("input[name='hyName']").val(hyName);
		$(parent.document).find("input[name='hyNo']").val(hyNo);
		parent.closeModal();
	}
