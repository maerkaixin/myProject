	...
	<body style="background:linear-gradient(white,#ebebeb,white);">
		...
		<div class="container">
			...
			<form class="form-horizontal text-center" name="infoForm" action="javascript:doSub()" method="post">
				...
				<input type="hidden" name="hcId" id="hcId" value="${hybk.hcId }">
				<input type="hidden" name="czId" id="czId" value="${cz.czId }">
				...
				<div class="container">
					<div class="form-group">
						...
						<div class="col-sm-6">
							<div class="form-group text-left">
								...
								<label class="control-label" id="hyName">${hybk.hyName }</label>
							</div>
							<div class="form-group text-left">
								<label class="control-label col-sm-3">卡类型名称 : </label>
								<label class="control-label" id="cardName">${hybk.cardName }</label>
							</div>
							<div class="form-group text-left">
								<label class="control-label col-sm-3">卡号 : </label>
								<label class="control-label" id="hcNo">${hybk.hcNo }</label>
							</div>
							<div class="form-group text-left">
								<label class="control-label col-sm-3">余额 : </label>
								<label class="control-label" id="hcNum">${hybk.hcNum }</label>
							</div>
						</div>
					</div>
					<hr/>
					<div class="form-group">
						...
						<div class="col-md-10 col-sm-10">
							<input class="form-control" name="czNum" value="${cz.czNum }"  placeholder="填写充值金额"
							 required="true" min="1" max="10000" step="1" type="number" />
						</div>
					</div>
					<div class="form-group">
						<ul class="pager">
...
<script type="text/javascript">
	...
	function tijiao(){
		bootbox.confirm("确认提交吗?",function(result){
			if (result) {
				$("form[name='infoForm']").attr("action","<%=ctx%>/cz/editsave.action");
				$("form").submit();
			}
		});
	}
