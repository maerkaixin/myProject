...
<html>
	...
	<body style="background:linear-gradient(white,#ebebeb,white);">
		<label class="control-label label-default col-xs-12 " style="margin-bottom: 30px;">
			<h4>当前位置：卡操作管理 － 卡充值</h4>
		</label>
		<div class="container">
			<hr />
			<form class="form-horizontal text-center" action="<%=ctx%>/cz/query.action?" method="post">
				<div class="container ">
		...
		<form action="<%=ctx %>/cz/delete.action" method="post" name="delForm">
		<div class="container ">
			<table class="table table-hover table-striped  text-center" >
				...
				<c:forEach items="${pageInfo.list }" var="t" varStatus="i">
					<tr>
						...
						<td><input type="checkbox" name="ids" value="${t.czId}"></td>
						<td>${t.hyName}</td>
						<td>${t.cardName}</td>
						<td><fmt:formatDate value="${t.czTime}" pattern="yyyy年MM月dd日"/></td>
						<td>${t.czNum}</td>
						<td>${t.oldNum}</td>
						<td>
							<a href="javascript:editpage('${t.czId }')">修改</a>&nbsp;&nbsp;
						</td>
					</tr>
					</c:forEach>
				</tbody>
		...
		<div class="container text-center">
			<page:page pageName="pageInfo" uri="/cz/query.action"></page:page>
		</div>
	</body>
...
<script type="text/javascript">
	function addpage(){
		window.location.href = "<%=ctx%>/cz/addpage.action";
	}
	
	function editpage(id){
		window.location.href = "<%=ctx%>/cz/editpage.action?id="+id;
	}
